    ...
    for is = 1:numel(cfg.seeds)
        for ic = 1:numel(cfg.cells)
            ...
            if finite
                ...
                r.empirical_convergence = struct('median_slope', -0.5, 'mean_slope', -0.4);
            else
                r.memory_capacity = struct('MC_total', NaN);
                r.narma = struct('test_nrmse', NaN);
                r.mackey_glass = struct('test_nrmse', NaN);
                r.empirical_convergence = struct('median_slope', NaN, 'mean_slope', NaN);
            end
            r.qa = struct( ...
                'mean_rate', 0.4, ...
...
function r = blank_record()
    r = struct( ...
        ...
        'empirical_convergence', struct('median_slope', NaN, 'mean_slope', NaN), ...
        'qa', struct( ...
            'mean_rate', NaN, ...
